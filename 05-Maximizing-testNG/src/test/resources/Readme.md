# 📂 TestNG Suite Configuration

This directory contains TestNG XML configuration files for organizing and executing automated test suites.

## Files

### 1. `Full Regression Suite`
**Location:** `/home/cauanricardoribeiro/Selenium/05-Maximizing-testNG/src/test/resources`

**Purpose:** Comprehensive regression testing suite covering positive and negative login scenarios.

**Test Structure:**
- **Positive Login Test** - Validates successful login functionality
- **Incorrect Username Test** - Tests login with invalid username
- **Incorrect Password Test** - Tests login with invalid password

**Key Features:**
- Uses parameterized testing for negative test cases
- Each negative test has specific error message validation
- All tests execute methods from `com.testautomation.login.LoginTests` class

### 2. `Smoke Test Suite`
**Location:** Same directory as above

**Purpose:** Quick smoke testing focused on critical login functionality.

**Test Structure:**
- Single **Login Test** that runs all methods tagged with `smoke` group
- Executes all test methods in `com.testautomation.login.LoginTests` class with `smoke` group annotation

## Test Methods Used

Both suites target methods in `com.testautomation.login.LoginTests` class:
- `testLoginFunctionality()` - Positive login test
- `testLoginNegative()` - Parameterized negative login tests

## Execution

Run these suites using TestNG runner, Maven, or your preferred test execution tool. The XML files define the test organization and parameters.

## Parameters for Negative Tests

Negative tests use these parameters:
- `username` - Test username
- `password` - Test password  
- `expectedErrorMessage` - Validation message for failed login
